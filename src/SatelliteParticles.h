...
public:
	...
	void add(const string& event, const string& actor, const string& repo) {
		...
		ofFloatColor c;
		if (event == "Push") c.set(.9, 1., 0.2);
		else if (event == "Create") c.set(0.9, 1., 0.1);
		...
		Particle p(ofRandom(0, 60), ofRandom(0, 60), ofRandom(0, 360), ofRandom(2, 5));
		p.col = c;
		p.repo = repo;
		p.user = actor;
	...
	void draw() {
		for (auto it = ps.begin(); it != ps.end(); it++) {
			ofDrawBitmapString(it->user, it->pos);
		}
	}
